        ...
        for entry in &self.entries {
            ...
            if entry.attrs.passthrough {
                let RouteInput::Token(t) = input else {
                    panic!(
                        "passthrough is only valid for token sources, this should've gotten caught by lower validation"
                    );
                };
    ...
    fn fire_target(
        ...
        match target {
            ...
            Target::InheritToken(target) => {
                let payload =
                    payload
                        .and_then(Payload::token)
                        .ok_or(RouteError::MissingPayload {
                            required: PayloadKind::Token,
                        })?;
                ctx.effects
                    .push(RouteEffect::Token(target.to_token(payload)));
                Ok(())
            }
            Target::Group(group) => {
...
fn match_token_pattern(pattern: &TokenPattern, token: &Token) -> Option<TokenPayload> {
    match pattern {
        TokenPattern::Key { key, mods } => match_key_pattern(key, mods, token),
    }
}
...
"#,
        ...
        assert_eq!(
            result.effects,
            vec![RouteEffect::Token(Token::press_key(
                Key::Enter,
                Mods::EMPTY
            ))],
        );
...
"#,
        ...
        assert_eq!(
            result.effects,
            vec![RouteEffect::Token(Token::press_utf8(
                'x',
                Mods::ALT | Mods::CTRL
            ))],
        );
...
"#,
        ...
        for mods in [
            ...
            assert!(
                fire_key(&router, Key::Function(1), mods).matched,
                "mods {mods:?} did not match"
            );
        }
    }
